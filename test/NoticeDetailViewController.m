...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == 3) {
        ...
        CGSize titleLabelSize = [self.sendContent sizeWithFont:forCal.font
                                             constrainedToSize:maximumLabelSize
        ...
        if (![self.sendReplySpecifyNotice isEqualToString:@""] && indexPath.row == 4) { // height for target cell
            titleLabelSize = [self.target sizeWithFont:forCal.font
                                     constrainedToSize:maximumLabelSize
                                         lineBreakMode:forCal.lineBreakMode];
        }
        else titleLabelSize = [[self.lstOfContent objectAtIndex:indexPath.row] sizeWithFont:forCal.font
                                                                          constrainedToSize:maximumLabelSize
                                                                              lineBreakMode:forCal.lineBreakMode]; // height for normal cell

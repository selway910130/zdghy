...
#pragma mark - add notation on tabbar

- (void)getNumOfUnreadNotice{ // get number of unread message from server
    @autoreleasepool {
...
- (void)addNotationOnTabBar{ // add notation icon and number on tabbar
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    CGSize titleLabelSize = [[tempNotice objectForKey:@"title"] sizeWithFont:[UIFont boldSystemFontOfSize:17]
...
// if cannot get detail, then do not jump to next view
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSDictionary *notice = [self.notices objectAtIndex:indexPath.item];
    self.noticeState = [notice objectForKey:@"specifyNoticeState"];
    
    if ([self.noticeState isEqualToString:@"0"]) {
        // remove unread icon or change the number on icon if user click item which have unread mark on it
        for (UIScrollView *subview in self.parentViewController.parentViewController.view.subviews){
            if (subview.tag == 3) {
